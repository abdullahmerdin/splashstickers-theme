...
assert(/\.wb-shell[\s\S]*background:\s*transparent/.test(builderCss) && /\.gb-stage-toolbar[\s\S]*background:\s*transparent/.test(builderCss) && /--wb-canvas-surround:\s*#ffffff/.test(builderCss), 'builder frame blends into the product page in both color modes');
...
assert(/\.gb-stage-toolbar[\s\S]*border:\s*0/.test(builderCss) && /\.gb-toolbar-divider\s*\{\s*display:\s*none/.test(builderCss) && /\.gb-inspector[\s\S]*border:\s*0/.test(builderCss) && /\.gb-purchase__total[\s\S]*border:\s*0/.test(builderCss), 'builder removes internal separator lines while preserving canvas guides');
